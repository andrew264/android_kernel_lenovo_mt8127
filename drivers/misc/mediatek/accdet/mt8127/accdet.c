...
#include <mach/eint.h>
//huangshiguo
#define HQ_35MM_TI_ENABLE //wangxiaoke 这里使用了新的欧美标切换芯片，GPIO也要作相应调整
//#define HQ_35MM_EARPHONE_TEST //121213 huangshiguo temp test
#define GPIO_HEADSET_SWITCH1 GPIO88
#define GPIO_HEADSET_SWITCH2 GPIO31 //wangciaoke  这里请参考电路图，如果需要使用HQ_35MM_EARPHONE_TEST这个宏必须修改这个GPIO，不能直接用
   ...
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		accdet_auxadc_switch(1);
    		accdet_adc = PMIC_IMM_GetOneChannelValue(ACCDET_ADC_CHANNEL,1,1);
		accdet_auxadc_switch(0);
	        ACCDET_DEBUG("[Accdet_lpz]adc accdet_voltage = %d mv\n", accdet_adc);

	        if(accdet_adc < 1000)//<500
	        {	
...
static int key_check(int b)
{
	...
	/* 0.24V ~ */
	ACCDET_DEBUG("accdet: come in key_check!!k\n");
	if((b<DW_KEY_HIGH_THR)&&(b >= DW_KEY_THR)) //  240----500
	{
		ACCDET_DEBUG("adc_data: %d mv\n",b);
		return DW_KEY;
	} 
	else if ((b < DW_KEY_THR)&& (b >= UP_KEY_THR))//90---240
	{
		ACCDET_DEBUG("adc_data: %d mv\n",b);
		return UP_KEY;
	}
	else if ((b < UP_KEY_THR) && (b >= MD_KEY_THR))//0---90
	{
		ACCDET_DEBUG("adc_data: %d mv\n",b);
		return MD_KEY;
	}
