...
/*----------------------------------------------------------------------------*/
#define CONFIG_MTK_AUTO_DETECT_ACCELEROMETER
...
static int bma222_remove_(void);
/*----------------------------------------------------------------------------*/
typedef enum {
    ADX_TRC_FILTER  = 0x01,
...
static int bma222_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(0 != (err = bma222_create_attr(&bma222_gsensor_driver.driver)))
	#else
...
#endif 
        //g_gsensor_name = BMA222_DEV_NAME;	
	loaded = 0;
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
...
static int bma222_i2c_remove(struct i2c_client *client)
{
	...
	#ifndef CONFIG_MTK_AUTO_DETECT_ACCELEROMETER
	if(0 != (err = bma222_delete_attr(&bma222_gsensor_driver.driver)))
	#else
...
{
	struct acc_hw *hw = get_cust_acc_hw2();
	GSE_FUN();
