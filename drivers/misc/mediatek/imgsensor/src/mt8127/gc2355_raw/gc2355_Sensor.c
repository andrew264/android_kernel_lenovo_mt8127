...
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);//yinliangliang add
...
static void GC2355_Sensor_Init(void)
{
	...
	GC2355_write_cmos_sensor(0xe3,0xff);  // c0 tt 12.15
	GC2355_write_cmos_sensor(0xe4,0x01);  // 0f tt 1.27
	GC2355_write_cmos_sensor(0xe5,0xff);
...
UINT32 GC2355GetSensorID(UINT32 *sensorID) 
{
	...
	*sensorID= ((GC2355_read_cmos_sensor(0xf0) << 8) | GC2355_read_cmos_sensor(0xf1));	
	...
	printk("[++++GC2355_adc++++] data0: %d , data1 = %d\n", data[0],data[1]);
	if(*sensorID == GC2355_SENSOR_ID && data[0] ==0){//huaquan:data[0] ==1,qunhui:data[0] ==0
   		return ERROR_NONE;  
	}else{
		*sensorID = 0xFFFFFFFF;	
		return ERROR_SENSOR_CONNECT_FAIL;
