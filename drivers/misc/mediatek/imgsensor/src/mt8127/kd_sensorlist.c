...
// @liguangxian20150310 
//kaka_12_0112_2 add
EXPORT_SYMBOL(g_main_camera);
EXPORT_SYMBOL(g_sub_camera);
//kaka_12_0112_2 end
/*******************************************************************************
...
static ssize_t CAMERA_HW_DumpReg_To_Proc2(struct file *file, char __user *pPage, size_t count,
					 loff_t *ppos)
{
    return count;
}
...
static ssize_t CAMERA_HW_Reg_Debug2(struct file *pFile, const char __user *buffer, size_t count,
				   loff_t *ppos)
{
    char regBuf[64] = {'\0'};
    u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
...
static const struct file_operations g_stCAMERA_camsensor_fops2 = {
	.owner = THIS_MODULE,
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    prEntry = proc_create("driver/camsensor2", 0, NULL, &g_stCAMERA_camsensor_fops2);
	if (!prEntry) {
