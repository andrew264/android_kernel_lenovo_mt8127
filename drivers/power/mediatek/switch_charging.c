...
void select_charging_curret_bcct(void)
{
	if ((BMT_status.charger_type == STANDARD_HOST) ||
		...
		g_temp_input_CC_value = CHARGE_CURRENT_1000_00_MA; //for 1900 1A
		...
		if (g_bcct_value < 550)
			...
			g_temp_CC_value = CHARGE_CURRENT_975_00_MA;//for 1900
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		//	printk("!!g_temp_input_CC_value=%d\n",g_temp_input_CC_value);
		//	printk("!!g_temp_CC_value=%d\n",g_temp_CC_value);
		//	mdelay(3);
		} else {
