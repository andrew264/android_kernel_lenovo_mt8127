...
if (BMT_status.temperature >= 57) {
	g_BatteryNotifyCode |= 0x0040;
	}
if (BMT_status.temperature <= -18) {	
	g_BatteryNotifyCode |= 0x0020;
				}
}
//////////////////
/*
	if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) {//MAX_CHARGE_TEMPERATURE
		g_BatteryNotifyCode |= 0x0002;
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too high)\n",
				    BMT_status.temperature);
		printk("[BATTERY] bat_temp(%d) out of range(too high)\n",BMT_status.temperature);
	}
	*/
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
/*
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too low)\n",
				    BMT_status.temperature);
		printk("[BATTERY] bat_temp(%d) out of range(too low)\n",BMT_status.temperature);
	}
*/
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
...
static void mt_battery_notify_UI_test(void)
{
	...
	} else if (g_BN_TestMode == 0x0006) {// 1900
		g_BatteryNotifyCode = 0x0020;
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0005_TOTAL_CHARGINGTIME\n");
	} else {
...
//////////runin
		if((NONSTANDARD_CHARGER == BMT_status.charger_type) && (second_check ==0))
                       {
                               second_check++;
                               mt_charger_type_detection();
                               battery_xlog_printk(BAT_LOG_CRTI, "[BAT_thread]CHR_Type_num=%d,second_check=%d\r\n", BMT_status.charger_type,second_check);
                       }
...

{
   ...
   if (copy_from_user(desc, buffer, len))
...

    {
        ...
        BMT_status.temperature = bat_tt_enable;  //写全局变量  
...
#define GT91XXG_ENABLE_CONFIG_PROC_FILE     "tempreture"

static struct proc_dir_entry *gt91xxg_enable_config_proc = NULL;



static const struct file_operations config_enable_proc_ops = {//操作函数集
...
int set_lenovo_charger_status()
{
	...
	//	if((g_tempStatus == 2) || ((g_tempStatus == 1) && (temperature >= 2)) || ((g_tempStatus == 3) && (temperature <= 8)))
			if((g_tempStatus == 2) || ((g_tempStatus == 1) && (temperature >= 1)) || ((g_tempStatus == 3) && (temperature <= 8)))
		{
			g_tempStatus = 2;
			#if 1
	...
	else if(temperature > 45 && temperature <= 50)
	{
		//if((g_tempStatus == 4) || ((g_tempStatus == 3) && (temperature >= 46)) || ((g_tempStatus == 5) && (temperature <= 48)))
			if((g_tempStatus == 4) || ((g_tempStatus == 3) && (temperature >= 46)) || ((g_tempStatus == 5) && (temperature <= 49)))
		{
			g_tempStatus = 4;
			#if 1
	...
	{   
		...
		case 2:
			{
				if (BMT_status.charger_type == STANDARD_CHARGER) 
					...
					set_bat_charging_current_limit(975);//for 1900
				else
		...
		case 4:
			{			
				if (BMT_status.charger_type == STANDARD_CHARGER) 
					//set_bat_charging_current_limit(750);
					set_bat_charging_current_limit(975);//for 960
				else 
					set_bat_charging_current_limit(500);
			   	break;
			}
